(function($){
    $.fn.themeRoller = function(){
        ...
        jquitr.addThemeRoller = function(parent){
            if ($('#inline_themeroller').size() > 0) {
                ...
                '</div>').appendTo(parent).draggable({
                    ...
                    cursor: 'resize-s',
                    bottom: '-3px'
                }).end().css('cursor', 'move').data("oldhash",window.location.hash).fadeIn();
            }
            jquitr.reloadCSS();
