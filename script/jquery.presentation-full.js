...
(function($) {
  $.fn.presentation = function(options) {
    ...
    $(this).each(function() {
      ...
      $presentation.addPageCounter=function(){
        $presentation.numSlides = $presentation.slides.length;
        
        if($presentation.currentHash) {
          $presentation.count = parseInt($presentation.currentHash);
        } else {
          $presentation.count = 1;
        }
        
        var slideCount="<span class='"
          + $presentation.options.pageCountClass+"'>"
          + (($presentation.count<10)?"0":"") 
          + $presentation.count
          + "/"
          + ($presentation.numSlides<10?"0":"")
          + $presentation.numSlides+"</span>";
          
        $presentation.append(slideCount);
      }
